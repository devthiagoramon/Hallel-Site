import { AddRounded, DoneRounded, Edit, ErrorRounded } from '@mui/icons-material';
import { Textarea } from '@mui/joy';
import { Box, Button, CircularProgress, MenuItem, Modal, Select, TextField } from '@mui/material';
...
const ModalEditDespesaEvento = (props) => {
    ...
    const editarDespesaEvento = () => {
        ...
        setEnviando(true);
        axios.put("http://localhost:8080/api/administrador/eventos/" + idEvento + "/despesa/" + parseInt(despesaSelected.id) + "/edit", {
            ...despesaEvento
        }, {
            headers: {
                    ...
                    <Box sx={styleInnerModal}>
                        ...
                        <div className='container_inputs_edit_despesas_evento'>
                            <div className='inputs_edit_despesas_evento'>
                                ...
                                <Textarea variant='outlined' sx={{background: "none"}} value={despesaEvento.descricao} maxRows={5} onChange={(e) => {
                                    setDespesaEvento(prevState => {
                                        return { ...prevState, descricao: e.target.value }
                                    })
                                }} />
                                <label>Tipo da despesa <span className='obrigatorio'>*</span></label>
                                <Select size='small' defaultValue={despesaEvento.num_tipoDespesa} value={despesaEvento.num_tipoDespesa} onChange={handleChangeSelect}>
                        ...
                        <div className='container_btn_edit_despesas_evento'>
                            <>
                                {enviando === false ?
                                    <Button variant='contained' endIcon={<Edit />} onClick={editarDespesaEvento}>Adicionar</Button> :
